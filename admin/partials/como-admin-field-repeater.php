...
?><ul class="repeaters"><?php
	...
	for ( $i = 0; $i <= $count; $i++ ) {
		...
		?><li class="<?php echo esc_attr( $setatts['class'] ); ?>">
			<div class="handle">
				<span class="title-repeater"><?php echo esc_html( $setatts['label-header'], 'como-consult' ); ?></span>
				<button aria-expanded="true" class="btn-edit" type="button">
					<span class="screen-reader-text"><?php echo esc_html( $setatts['label-edit'], 'como-consult' ); ?></span>
				...
				<div class="wrap-fields"><?php
					...
					foreach ( $setatts['fields'] as $fieldcount => $field ) {
						...
						foreach ( $field as $type => $atts ) {
							...
							include( plugin_dir_path( __FILE__ ) . 'como-admin-field-' . $type . '.php' );
				...
				<div>
					<a class="link-remove" href="#">
						<span><?php
							...
							echo esc_html( apply_filters( $this->plugin_name . '-repeater-remove-link-label', $setatts['label-remove'] ), 'como-consult' );
	...
	?><div class="repeater-more">
		...
		<a class="button" href="#" id="add-repeater"><?php
			...
			echo esc_html( apply_filters( $this->plugin_name . '-repeater-more-link-label', $setatts['label-add'] ), 'como-consult' );
